...
const FlyerFormat = ({ product, onDetailsProprieterChange }) => {
  ...
  useEffect(() => {
    if (product?.proprieter) {
      const initialDetails = product.proprieter.map((prop) => ({
        label: prop.name_proprieter,
        value: "-",
      }));

      setDetailsState((prev) => {
        const merged = [...initialDetails,...prev];
        onDetailsProprieterChange(merged);
        return merged;
      });
    }
  }, [product]);
  ...
  const handleSelectOption = (proprieterName, optionId) => {
    setDataPost((prev) => ({
      ...prev,
      [proprieterName]: optionId, // نخزن id ديال option
    }));
  }; 
  ...
  if (hasMissing) return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Server error:", text); // سيطبع HTML الخطأ
        return;
      }
